...
function BasketItem(props) {
  ...
  const onClickRemoveBook = useCallback(
    () => {
      dispatch(removeBookFromCart({bookId: props.book.id, bookAmount: props.book.amount}));
    },
  [props.book.id, dispatch],
  );
  ...
  return(
    <div className="open-basket__item">
      <div className="open-basket__book-counter">
        <button className="open-basket__plus-counter--button" onClick={onClickPlusButton}>
          <FontAwesomeIcon icon={faPlus} color="#rgb(119, 121, 140)"/>
        </button>
        <div className="open-basket__book-count">
          {props.book.amount}
        </div>
        <button className="open-basket__minus-counter--button" onClick={onClickMinusButton}>
      ...
      <div className="open-basket__book-details">
        <div className="open-basket__book-title">
          {bookDetails.title}
        </div>
      ...
      <div className="open-basket__book-total-price">
        ${bookTotalPrice}
      </div>
      <div className="open-basket__book-cancelation">
        <button className="open-basekt__close--button" onClick={onClickRemoveBook}>
